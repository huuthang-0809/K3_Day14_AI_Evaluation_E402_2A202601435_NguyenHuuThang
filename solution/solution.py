...
class EvalResult:
    ...
    def overall_score(self) -> float:
        ...
        Return the mean of the three answer-quality metrics. Retrieval-side
        metrics are intentionally excluded from this aggregate.
        """
...
class LLMJudge:
    ...
    def score_response(
        ...
        prompt = (
            "Score the following answer from 0 to 1 for each rubric criterion. "
            "Return a JSON object with a 'scores' object and a 'reasoning' string.\n\n"
            f"Question:\n{question}\n\nAnswer:\n{answer}\n\n"
            f"Rubric:\n{criteria}"
        )
        ...
        try:
            ...
            scores = {
                name: max(0.0, min(1.0, float(parsed_scores[name])))
                for name in rubric
                if name in parsed_scores
            }
    ...
    def detect_bias(self, scores_batch: list[dict[str, Any]]) -> dict[str, Any]:
        ...
        for item in scores_batch:
            ...
            if isinstance(item, dict) and "first_score" in item and "second_score" in item:
                first_scores.append(float(item["first_score"]))
                second_scores.append(float(item["second_score"]))

        positional_bias = bool(first_scores and sum(first_scores) / len(first_scores) >
...
class BenchmarkRunner:
    ...
    def run_regression(self, new_results: list, baseline_results: list) -> dict:
        ...
        def averages(results: list) -> dict[str, float]:
            return {
                name: (
                    sum(getattr(result, name) for result in results) / len(results)
                    if results else 0.0
                )
        ...
        return {
            **{f"new_avg_{name}": new_avg[name] for name in metric_names},
            **{f"baseline_avg_{name}": baseline_avg[name] for name in metric_names},
...
class FailureAnalyzer:
    ...
    def generate_improvement_suggestions(
        ...
        suggestion_by_type = {
            "hallucination": "Add a hallucination checker and require every claim to be supported by retrieved context",
            "irrelevant": "Improve intent detection and add prompt instructions to answer the user's question directly",
            "incomplete": "Increase useful context coverage and add examples showing complete answers",
            "off_topic": "Add topic checks and clarify the answer scope in the system prompt",
            "refusal": "Review safety guardrails so valid requests are not refused unnecessarily",
        }
        ...
        if len(suggestions) < 3:
            suggestions.extend([
                "Review retrieved chunks and improve ranking of relevant evidence",
                "Add representative golden cases and regression checks to CI",
                "Calibrate evaluation thresholds against human-labeled examples",
            ])
